    ...
    public void utiliserPouvoirAvatar() {
        ...
        do {
            ...
            if (choix >= 2 && choix <= 8) {
                ...
                int i = 0;
                do {
                    if (this.getPlateau().getPersonnage(i).getNom().equals(tableauPersonnage[choix-1]) && this.getPlateau().getPersonnage(i).getRang()!=1){
                        this.getPlateau().getPersonnage(i).setVole();
                        this.getJoueur().ajouterPieces(this.getPlateau().getPersonnage(i).getJoueur().nbPieces());
                        this.getPlateau().getPersonnage(i).getJoueur().retirerPieces(this.getPlateau().getPersonnage(i).getJoueur().nbPieces());
                        //System.out.println("ahah "+this.getJoueur().nbPieces());
                        condition = false;
                    }else if (this.getPlateau().getPersonnage(i).getNom().equals(tableauPersonnage[choix-1]) && this.getPlateau().getPersonnage(i).getRang()==1){
